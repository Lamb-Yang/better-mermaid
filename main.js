...
var MermaidImageModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    this.sizeModeBtn = controls.createEl("button", {
      text: this.t("actualSize"),
      cls: "better-mermaid-size-mode-btn"
    });
    this.sizeModeBtn.addEventListener("click", () => {
      this.toggleSizeMode();
    });
    this.applySizeMode();
    this.applyTransform();
  ...
  toggleSizeMode() {
    this.sizeMode = this.sizeMode === "fit" ? "actual" : "fit";
    this.scale = 1;
    this.panX = 0;
    this.panY = 0;
    this.applySizeMode();
  ...
  applySizeMode() {
    if (!this.svgEl || !this.sizeModeBtn)
      return;
    if (this.sizeMode === "actual") {
      const size = this.getSvgNaturalSize();
      ...
      this.svgEl.addClass("better-mermaid-svg-actual");
      this.sizeModeBtn.setText(this.t("fitToModal"));
    } else {
      this.svgEl.style.width = "";
      this.svgEl.style.height = "";
      this.svgEl.style.maxWidth = "";
      this.svgEl.style.maxHeight = "";
      this.svgEl.removeClass("better-mermaid-svg-actual");
      this.sizeModeBtn.setText(this.t("actualSize"));
    }
  }
  getSvgNaturalSize() {
    ...
    const attrW = parseFloat(this.svgEl.getAttribute("width") || "");
    const attrH = parseFloat(this.svgEl.getAttribute("height") || "");
    if (attrW > 0 && attrH > 0) {
...
.better-mermaid-modal-size {
  width: min(${this.settings.modalWidthPercent}vw, calc(100vw - 32px));
  height: min(${this.settings.modalHeightPercent}vh, calc(100vh - 32px));
