...
在css snippets中单独做了一份控制样式，直接复制保存成文件，搭配插件一起使用更佳。
...
**适配弹窗模式：**
```css
.better-mermaid-viewport svg {
  max-width: calc(100% - 24px);
  max-height: calc(100% - 48px);
}
```

SVG 由 CSS 限制在视口内，`width: auto` / `height: auto` 等比缩放。

**原始大小模式：**

```javascript
const size = this.getSvgNaturalSize();  // 读取 viewBox 或属性
this.svgEl.style.width = `${size.width}px`;
this.svgEl.style.height = `${size.height}px`;
this.svgEl.style.maxWidth = 'none';
this.svgEl.style.maxHeight = 'none';
```

取消 CSS 限制，按 SVG 固有像素渲染。超出视口部分通过 `overflow: hidden` + transform 拖拽查看。

**固有尺寸探测优先级：**

1. `viewBox` 属性（Mermaid 输出通常带这个）：`viewBox="0 0 800 600"` → 800×600
2. `width` / `height` 属性
3. `getBBox()` DOM API（部分手动创建的 SVG 节点）
4. 回退到 800×600（极少数无尺寸信息的 SVG）

### 缩放与变换

使用 CSS `transform: translate(x, y) scale(s)`，`transform-origin: 0 0` 保持与 viewBox 坐标一致。

缩放以鼠标指针为中心点，计算平移偏移量保持视觉连续性：
...
1. 克隆原始 SVG 节点
2. 从 viewBox 读取固有尺寸写入 width/height
3. XMLSerializer 序列化 → data URL
4. 绘制到 2x 分辨率 Canvas（保证导出清晰度）
5. 自动创建下载链接触发保存
